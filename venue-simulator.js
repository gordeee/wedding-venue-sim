...
class VenueSimulator {
    ...
    createProfessionalLayout() {
        ...
        const daisX = this.venue.daisX;  // 24ft
        const daisY = this.venue.daisY;  // 36.5ft
        const daisR = this.venue.daisRadius;  // 12.5ft

        // Venue bounds: 62ft wide × 67ft tall
        // Dais bottom edge: y = 36.5 + 12.5 = 49ft
        // Available space below dais: 67 - 49 = 18ft

        const minClearance = 4; // feet from dais edge
        const tableSpacing = 4; // feet between tables

        // SECTION 1: Premium front seating - close to dais
        // Two 8ft banquet tables flanking center aisle
        this.addTable('banquet8', 12, 54, 0);  // Left front
        this.addTable('banquet8', 36, 54, 0);  // Right front

        // SECTION 2: Mid-section - create balanced rows
        // Left side with 6ft tables
        this.addTable('banquet6', 10, 60, 0);  // Left mid
        this.addTable('banquet6', 22, 60, 0);  // Center-left mid

        // Right side with 6ft tables
        this.addTable('banquet6', 34, 60, 0);  // Center-right mid
        this.addTable('banquet6', 46, 60, 0);  // Right mid

        // SECTION 3: Serpentine accent tables for visual variety
        // Create elegant curved sections on sides
        this.addTable('serpentine', 6, 54, 0);   // Left serpentine facing inward
        this.addTable('serpentine', 48, 54, 270); // Right serpentine facing center

        // SECTION 4: Above the dais - small intimate area
        // Place a few tables in the 24ft space above dais
        this.addTable('banquet6', 12, 20, 0);  // Upper left
        this.addTable('banquet6', 36, 20, 0);  // Upper right

        // SECTION 5: Side accent tables
        // Small tables for intimate groups
        this.addTable('banquet6', 24, 12, 0);  // Top center
